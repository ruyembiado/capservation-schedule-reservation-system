...
class AdminController extends Controller
{
	...
	{
		// --- Get Instructors ---
		$instructorRecords = User::where('user_type', 'panelist')->get();
		$instructors = [];
		...
		$panelists = User::whereNot('id', $group_id)->where('user_type', 'panelist')->where(function ($query) use ($tags) {
			foreach ($tags as $tag) {
		...
		return view('smartscheduler', compact(
			'formatted',
			'group_id',
			'panelists',
			'dean_name',
		...
		foreach ($reservation_ids as $i => $reservationId) {
			...
			if ($schedule) {
				...
				try {
					...
					$panelistSent = $scheduleController->sendPanelistEmail($reservation, $panelist_ids);
					$deanSent     = $scheduleController->sendDeanHeadEmail($reservation);

					if (!$panelistSent || !$deanSent) {
						Log::warning('Email sending failed for reservation ID: ' . $reservation->id);
					}
				} catch (\Throwable $e) {
	...
	{
		$panelists = User::where('user_type', 'panelist')->orderBy('created_at', 'desc')->get();
