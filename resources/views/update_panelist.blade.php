...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0">Edit Panelist</h1>
    </div>
    <div class="card shadow col-12 mb-4">
        <div class="card-body">
            <form action="{{ route('instructor.update', $panelist->id) }}" method="POST">
                @csrf
                    ...
                    <div class="col-6">
                        ...
                        <div class="col-12 mb-2">
                            ...
                            <div id="credentialsRepeater">
                                @php
                                    $credentials = old('credentials', json_decode($panelist->credentials) ?? ['']);
                                @endphp
                    ...
                    <div class="col-5">
                        <!-- <div class="col-12 mb-2">
                            ...
                            <div id="vacantTimeRepeater">
                                @php
                                    $vacantTimes = old(
                                        'vacant_time.day',
                                        json_decode($panelist->vacant_time, true) ?? [''],
                                    );
                                @endphp
                        ...
                        <div class="col-12 mb-2">
						    ...
						    <select name="position" id="position" 
						        ...
						        <option value="BSIT Course Instructor" {{ $panelist->position == 'BSIT Course Instructor' ? 'selected' : '' }}>BSIT Course Instructor</option>
						        <option value="BSCS Course Instructor" {{ $panelist->position == 'BSCS Course Instructor' ? 'selected' : '' }}>BSCS Course Instructor</option>
						        <option value="BSIS Course Instructor" {{ $panelist->position == 'BSIS Course Instructor' ? 'selected' : '' }}>BSIS Course Instructor</option>
						        <option value="Panelists" {{ $panelist->position == 'Panelists' ? 'selected' : '' }}>Panelists</option>
						    </select>
						    @error('position')
						        <div class="invalid-feedback d-block">{{ $message }}</div>
						    @enderror
